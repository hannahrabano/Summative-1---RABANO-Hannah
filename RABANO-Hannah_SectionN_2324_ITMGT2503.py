...
import json
with open(r"D:\Hannah\college\ITMGT\LT\script.json", 'r') as file:
...
for num in range(0, length-1+1):
    key = str(num)
    new_value = jsondata[key].upper()
    jsondata.update({key:new_value})
...
for num in range(0, length-1+1):
    ...
    for text in remove_text:
        new_value = jsondata[key].replace(text, '')
        jsondata.update({key:new_value})


for num in range(0, length-1+1):
    key = str(num)
    for text in unnecessary_punctuation:
        new_value = jsondata[key].replace(text, ' ')
        jsondata.update({key:new_value})
...
for word in all_words:
    if not(word in words_list_unique):
        words_list_unique.append(word)

for word in words_list_unique:
    if len(word) >= 3:
        wordcount_dictionary.update({word : all_words.count(word)})
...
keys_wordcount_dictionary = list(wordcount_dictionary.keys())

for index in range(0, len(wordcount_dictionary)-1+1):
    word = keys_wordcount_dictionary[index]
    tuple_word_count = tuple([word, wordcount_dictionary[word]])
    final_wordlist_of_tuples.append(tuple_word_count)
...
# Power: The power of the raiding party will dictate the level of the raid’s success. 
# Each Fighter gives 10 points, each Ranger gives 12 points, each Cleric gives 16 points.

# The King’s military advisors have also discussed strategies that will be employed in the raid: 
# Having more Fighters than Rangers will bolster the raiding party's frontline defense To avoid casualties, 
# each Cleric must have at most 3 Fighters that they are supporting


# Z == 10*F + 12*R + C*16

# 1000F + 600R + 750C <= 18000 
# 500F + 750R + 300C <= 12000
# 50F + 45R + 75C <= 1500 
...
for C in range(0, 20+1):
    for R in range(0, 16+1):
        for F in range(0, C**3+1):
            Z_trial = 10*F + 12*R + C*16
...
def Collatz(start_number):
    ...
    collatz_data = {}
    collatz_data.update({'collatz_sequence':sequence})
    collatz_data.update({'collatz_length':len(sequence)})
    collatz_data.update({'max_collatz':max(sequence)})

    sum = 0
    for num in sequence:
        sum += num

    collatz_data.update({'sequence_sum':sum})    
    ...
    for n in number_list:
        ...
        while num != 1:
            if num % 2 == 0:
                ...
                num = int(3*num + 1)
            indiv_sequence.append(num)

        collatz_length = len(indiv_sequence)
        max_collatz = max(indiv_sequence)
        
        all_sequences.update({n : [collatz_length, max_collatz]})

        indiv_sequence = []

    numbers_list = list(all_sequences.keys())
    data_list = list(all_sequences.values())
    
    length_list = []
    for index in range(0, len(all_sequences)-1+1):
        key = numbers_list[index]
        length_list.append(all_sequences[key][0])
    highest_length = max(length_list)

    max_list = []
    for index in range(0, len(all_sequences)-1+1):
        key = numbers_list[index]
        max_list.append(all_sequences[key][1])
    highest_max = max(max_list)

    if [highest_length, highest_max] in data_list:
        index = data_list.index([highest_length, highest_max])
